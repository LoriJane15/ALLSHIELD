            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        <div class="me-3">
                            <a href="{{ route('admin.rcsp.show', [$bgy, 'phase' => $form->phase?->number]) }}"
                        ...
                        <div class="file-info ms-3">
                            <h5 class="mb-1">File ID: {{ $form->id }}</h5>
        ...
        <div class="col-12">
            <div class="status-container d-flex flex-wrap gap-3">
                ...
                <div class="status-card" style="background-color: #ebf0ff;">
                    ...
                    <div class="value">{{ $form->created_at?->format('F j, Y g:i A') }}</div>
                </div>
            </div>
        </div>
    </div>

    <!-- PDF Viewer and Comments Section -->
    <div class="row">
        <!-- Viewer -->
        <div class="col-md-8">
                    ...
                    @if (! $fileUrl)
                        <div class="d-flex align-items-center justify-content-center h-100" style="background:#f8f9fa;">
                            <div class="text-center p-5">
                                <i class="fa fa-file-o fa-5x text-secondary mb-3"></i>
                    ...
                    @else
                        <div class="d-flex align-items-center justify-content-center h-100" style="background:#f8f9fa;">
                            <div class="text-center p-5">
                                ...
                                <p class="mb-3"><span class="badge bg-light text-dark"><i class="fa fa-file me-1"></i>{{ strtoupper($ext) }} File</span></p>
                    ...
                    <div id="commentsList" data-form-id="{{ $form->id }}" data-fallback-avatar="{{ $fallbackAvatar }}">
                        @forelse ($form->fileComments->sortByDesc('id') as $c)
                            ...
                            <div class="comment-card mb-3">
                                <div class="d-flex gap-2 {{ $isAdmin ? '' : 'justify-content-end' }}">
                                    @if ($isAdmin)
                                    ...
                                    <div class="flex-grow-0">
                                        <div class="comment-content p-3 {{ $isAdmin ? 'bg-light' : 'bg-primary text-white' }} rounded" style="max-width: 80%;">
                                            <p class="mb-1">{{ $c->text }}</p>
                                            <small class="{{ $isAdmin ? 'text-muted' : 'text-white-50' }}">
                                                {{ $c->user?->name ?? 'User' }} · {{ $c->created_at?->format('F j, Y g:i A') }}
                                            </small>
                                        </div>
                                    </div>
                                    @if (! $isAdmin)
