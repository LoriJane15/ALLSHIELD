...
<div class="mblrc-dashboard-container">
            ...
            <div class="d-flex align-items-center" style="gap: 1.25rem;">
                ...
                <div>
                    <div class="mblrc-hero-eyebrow">
                        PSWDO Program &middot; Social Welfare Intake Records
                    </div>
                    <h1 class="mblrc-hero-title">PSWDO Enrollment Documents</h1>
                    <p class="mblrc-hero-sub">Authoritative enrollment dossiers, social case studies, and municipal endorsements.</p>
                </div>
            </div>
            <div>
                <span class="badge" style="background: rgba(255,255,255,0.15); color: #ffffff; padding: 0.5rem 1rem; font-size: 0.8125rem; font-weight: 750; border-radius: 8px; backdrop-filter: blur(4px); border: 1px solid rgba(255,255,255,0.2);">
                    <i class="mdi mdi-shield-check mr-1"></i> Verified Repository
                </span>
            </div>
        </div>
    </div>
    ...
    {{-- Main Document Card --}}
    <section class="mblrc-form-card mb-4" aria-labelledby="pswdo-docs-heading">
        <div class="mblrc-form-card-header d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center gap-2">
                <i class="mdi mdi-file-document-outline" style="font-size: 1.25rem; color: #312e81;"></i>
                <h2 id="pswdo-docs-heading" class="mb-0" style="font-size: 0.95rem; font-weight: 800; color: #0f172a;">
                    Enrollment File Archive
                </h2>
            </div>
            @if(count($documents) > 0)
                <span class="badge" style="background: #eef2ff; color: #312e81; font-weight: 800; border-radius: 6px; padding: 0.35rem 0.75rem;">
                    {{ count($documents) }} Attached {{ count($documents) === 1 ? 'File' : 'Files' }}
                </span>
            @endif
        </div>
        <div class="mblrc-form-card-body p-3">
                ...
                <div class="record-doc-item">
                    <div class="d-flex align-items-center" style="gap: 1rem; min-width: 0;">
                        ...
                        <div style="min-width: 0;">
                            <h3 class="mb-1 font-weight-bold" style="color: #0f172a; font-size: 0.95rem;">{{ $document['label'] }}</h3>
                            <p class="small text-muted mb-0">
                                <i class="mdi mdi-calendar-clock mr-1" style="color: #312e81;"></i>
                                <strong>{{ $document['date']['label'] ? $document['date']['label'].':' : '' }}</strong>{{ $document['date']['label'] ? ' ' : '' }}{{ $document['date']['value'] }}
                            </p>
                        </div>
                    </div>
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        <a class="btn btn-sm" href="{{ $document['previewUrl'] }}" style="background: #312e81; color: #ffffff; font-weight: 750; border-radius: 8px; padding: 0.45rem 1rem; border: none; box-shadow: 0 2px 6px rgba(49, 46, 129, 0.18);">
                            <i class="mdi mdi-eye-outline mr-1"></i> Secure preview
                        </a>
                        <a class="btn btn-sm" href="{{ $document['downloadUrl'] }}" style="background: #ffffff; color: #0f172a; font-weight: 750; border-radius: 8px; padding: 0.45rem 1rem; border: 1px solid #cbd5e1;">
            ...
            @empty
                <div class="text-center py-5 text-muted">
                    ...
                    <small class="text-muted">No official PSWDO enrollment files have been uploaded for this case profile.</small>
                </div>
            @endforelse
        </div>
    </section>
    ...
    <div class="record-security-note">
        ...
        <div>
            <strong>Restricted Access &middot; Encrypted Storage:</strong> All documents in this repository are subject to strict data-privacy protocols, verified against tamper-evident SHA-256 checksums, and encrypted at rest with AES-256.
        </div>
    </div>
</div>
@endsection
