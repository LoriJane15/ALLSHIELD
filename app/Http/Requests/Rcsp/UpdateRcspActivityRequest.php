...
class UpdateRcspActivityRequest extends FormRequest
{
    ...
    public function authorize(): bool
    {
        ...
        return $barangay && $activity
            && ($this->user()?->can('updateActivity', [$barangay, $activity]) ?? false);
    }
    ...
    public function after(): array
    {
        return [function (Validator $validator): void {
            if (array_diff(array_keys($this->all()), ['_token', '_method', 'title'])) {
                $validator->errors()->add('request', 'Unknown activity fields are not accepted.');
            }

            $barangay = $this->route('rcspBarangay');
            $activity = $this->route('activity');
            if (! $barangay || ! $activity) {
                return;
            }
            ...
            if (RcspActivity::where('rcsp_barangay_id', $barangay->id)
                ...
                ->whereKeyNot($activity->id)->exists()) {
